$(function () {
    layui.use(['element', 'table', "form", "layer"], function () {
        ...
        render_white_ip_table(netcard)

        // 传入网卡名，将IP黑名单渲染到表格
        function render_white_ip_table(netcard) {
            if (netcard === "default") {
                layer.msg("请选择一张网卡")
                // 渲染空表格
                table.render({
                    ...
                    cols: [[
                        {field: 'white_ip', title: 'IP黑名单', width: "40%", sort: false},
                    ...
                    if (res.code === 200) {
                        if (res.data.data === null) {
                            return {
                                ...
                                "data": [{"white_ip": "-", "hit": "-", "option": "-"}] //解析数据列表
                            };
                        }
                        for (var i = 0; i < res.data.data.length; i++) {
                            interface_data.push({
                                "white_ip": res.data.data[i].ip,
        ...
        // 监听删除按钮
        table.on('tool(white_ip_table)', function(obj){
            // console.log(obj)
            var netcard = $("#netcard-select-input").val()
            var target = [obj.data.white_ip.replace("/32", ""), ]
            var del_data = {
                "iface": netcard,
                "whiteIpList": target
            }
