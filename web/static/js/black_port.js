$(function () {
    layui.use(['element', 'table', "form", "layer"], function () {
        ...
        render_black_port_table(netcard)

        // 传入网卡名，将IP黑名单渲染到表格
        function render_black_port_table(netcard) {
            if (netcard === "default") {
                layer.msg("请选择一张网卡")
                // 渲染空表格
                table.render({
                    elem: "#black_port_table",
                    page: {
                        limit: 10,
                        limits: [10, 20, 30],
                    },
                    cols: [[
                        {field: 'black_port', title: '端口黑名单', width: "40%", sort: false},
                        {field: 'hit', title: '命中次数', width: "40%", sort: false},
                        {field: 'option', title: "操作", width: "20%", sort: false, align: "center"},
                    ]],
                    data: [
                        {"black_port": "-", "hit": '-', "option": "-"}
                    ]
                })
                return
            }
            table.render({
                    ...
                    if (res.code === 200) {
                        if (res.data.data === null) {
                            return {
                                ...
                                "data": [{"black_port": "-", "hit": "-", "option": "-"}] //解析数据列表
                            };
                        }
                        for (var i = 0; i < res.data.data.length; i++) {
                            interface_data.push({
                                "black_port": res.data.data[i].port,
                ...
                cols: [[
                    {field: 'black_port', title: 'IP黑名单', width: "40%", sort: false},
                    {field: 'hit', title: '命中次数', width: "40%", sort: false},
        ...
        form.on("select(select_changed)", function (data) {
            ...
            render_black_port_table(data.value)
        })

        // 监听删除按钮
        table.on('tool(black_port_table)', function(obj){
            // console.log(obj)
            var netcard = $("#netcard-select-input").val()
            var target = [obj.data.black_port, ]
            var del_data = {
                "iface": netcard,
                "blackPortList": target
            }
            console.log("删除一个 [黑名单Port]", del_data)
            $.ajax({
                url: "/xdp/black/delPort",
                type: "post",
